...
export default function initScene(gl: WebGL2RenderingContext): Scene {
    ...
    // Generate the drawables for the cityscape
    ...
    // Prepare the convolutional neural network shader
    ...
    // A green cube just to show where the origin is
    const origin = new Cube(gl);
    const drawVegFunc = RenderUtils.drawFunction(gl, camera, mainProgram, LabelColors.VEGETATION);
    const originDrawer = new Drawer(origin, drawVegFunc);

    // TODO: Remove skybox when neural network ready
    ...
    // Order of render target drawing
    const renderTargets = [
        sceneRenderTarget,
        postRenderTarget,
    ];

    // Create sequenced renderer that renders the scene and then the post-processing quad
    const sequencedRenderer = new SequencedRenderer(renderTargets);
    sequencedRenderer
        .genAndAddRenderables(
            sceneRenderTarget,
            [
                originDrawer,
                ...buildings,
    ...
    // Create a scene that updates the camera and draws the sequenced renderer.
    ...
    // This is the first 7x7x4 convolutional layer of the neural network. Order is left-right, top-bottom.
    // The x4 comes from the fact that each 4 consecutive values represents the weight of the rgba channels.
    // If each group is unpacked into a vec4 in the shader, one can easily take the dot product to get the
    // weighted sum for that pixel location.
    // TODO: Automate this values into the shader once confident the data layout is correct.
